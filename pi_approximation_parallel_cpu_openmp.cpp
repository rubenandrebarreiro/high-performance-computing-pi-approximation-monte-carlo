// Aproximacao de PI (com animacao) - Versao Paralela em CPU com OpenMP
...
// Importacao da biblioteca OpenMP (Open Multi-Processing)
// para computacao paralela em CPU (Central Processing Unit)
...
int main() {
    ...
    // baseados em Mersenne Twister com periodos longos de 2^(19937)-1
    ...
    sf::RenderWindow window(sf::VideoMode(800, 600),
                                          ...
                                          "(Versao Paralela em CPU com OpenMP)");
    ...
    while(window.isOpen() && current_point < NUM_MAX_POINTS &&
        ...
        // Definicao do numero de pontos a serem gerados
        // individualmente por cada batch (lote) associado
        // a um thread/worker (trabalhador) local
        constexpr int points_per_batch = 10000;

        // Definicao do numero de pontos interiores a circunferencia,
        // referentes a cada thread/worker (trabalhador) local
        int local_inside_circle = 0;

        // Definicao do numero de pontos exteriores a circunferencia,
        // referentes a cada thread/worker (trabalhador) local
        int local_outside_circle = 0;


        // Inicialização do vetor de pontos interiores a circunferencia,
        // referentes a cada thread/worker (trabalhador) local
        std::vector<sf::CircleShape> local_worker_inside_points;

        // Inicialização do vetor de pontos exteriores a circunferencia,
        // referentes a cada thread/worker (trabalhador) local
        std::vector<sf::CircleShape> local_worker_outside_points;


        // Definicao de uma regiao de ciclo paralelo
        // para padrao de reducao, a ser executada individualmente
        // por cada thread/worker (trabalhador) local lancado
        #pragma omp parallel for reduction(+:local_inside_circle)
        // Para cada ponto a serem gerado por cada batch (lote)
        // associado a um thread/worker (trabalhador) local
        for (int i = 0; i < points_per_batch; i++) {
            ...
            if (x * x + y * y <= RADIUS * RADIUS) {
                ...
                // Definicao de regiao critica para execucao individual
                // por parte de cada thread/worker (trabalhador) local,
                // para a instrucao de insercao de um ponto interior a circunferencia
                #pragma omp critical

                // Adicao do ponto base gerado aleatoriamente
                // por cada thread/worker (trabalhador) local,
                // ao vetor de pontos interiores a circunferencia,
                // referentes a cada thread/worker (trabalhador) local
                local_worker_inside_points.push_back(point);

                // Incremento do numero de pontos interiores a circunferencia,
                // referentes a cada thread/worker (trabalhador) local
                local_inside_circle++;
            ...
            // (ou seja, o ponto atual gerado e um ponto externo)
                ...
                // Definicao de regiao critica para execucao individual
                // por parte de cada thread/worker (trabalhador) local,
                // para a instrucao de insercao de um ponto exterior a circunferencia
                #pragma omp critical

                // Adicao do ponto base gerado aleatoriamente
                // por cada thread/worker (trabalhador) local,
                // ao vetor de pontos exteriores a circunferencia,
                // referentes a cada thread/worker (trabalhador) local
                local_worker_outside_points.push_back(point);

                // Definicao do numero de pontos exteriores a circunferencia,
                // referentes a cada thread/worker (trabalhador) local
                local_outside_circle++;

            }

        }

        // Somar o numero de pontos interiores a circunferencia,
        // referentes a cada thread/worker (trabalhador) local,
        // ao numero de pontos interiores a circunferencia global
        inside_circle += local_inside_circle;


        // Adicionar o vetor de pontos interiores a circunferencia,
        // referentes a cada thread/worker (trabalhador) local,
        // ao vetor de todos os pontos interiores a circunferencia global
        inside_points.insert(inside_points.end(),
                             local_worker_inside_points.begin(),
                             local_worker_inside_points.end());

        // Adicionar o vetor de pontos exteriores a circunferencia,
        // referentes a cada thread/worker (trabalhador) local,
        // ao vetor de todos os pontos exteriores a circunferencia global
        outside_points.insert(outside_points.end(),
                              local_worker_outside_points.begin(),
                              local_worker_outside_points.end());

        // Soma do indice correspondente ao ponto atual gerado,
        // tendo em consideracao o numero de pontos gerados local
        current_point += points_per_batch;

        // Atualizacao da aproximacao de PI atual,
        // considerando o ponto atual gerado
        pi_approx = 4.0 * inside_circle / current_point;
        ...
        pi_approx_string_stream << "Estimativa/Aproximacao de PI: " << pi_approx;
